...
public static class ArmGeom {
    public const double VehR = 4.73, RailR = 0.6, Inset = 1.2, HingeZ = 6.53, Reach = 34.1, Length = 42;
    public const double Ready = Const.ARM_GAP_READY, Park = Const.ARM_GAP_PARK;
    public static double Angle(double gap) {
        double c = HingeZ + Math.Max(gap, 0), r = Math.Sqrt(Reach * Reach + HingeZ * HingeZ);
        return Math.Asin(Math.Min(c / r, 1)) - Math.Atan2(HingeZ, Reach);
    }
