...
public sealed class WorldRig {
    ...
    public static WorldRig Build(Node3D world) {
        ...
        w._ground = new MeshInstance3D { Mesh = new PlaneMesh { Size = new Vector2((float)(30000 + Const.COAST_DR), 60000) } };
        w._ground.MaterialOverride = new StandardMaterial3D {
            AlbedoColor = new Color(0.20f, 0.19f, 0.17f), Roughness = 0.95f, Metallic = 0f,
        };
        w._ground.Position = new Vector3(LandMid, -PadView.DeckH - 0.05f, 0);
        world.AddChild(w._ground);
        w._beach = new MeshInstance3D { Mesh = new PlaneMesh { Size = new Vector2(70, 60000) } };
        w._beach.MaterialOverride = new StandardMaterial3D {
    ...
    public void Update(SimState sim, Origin org) {
        ...
        _ground.Position = new Vector3(LandMid - (float)org.X, -PadView.DeckH - 0.05f - (float)org.Y, 0);
